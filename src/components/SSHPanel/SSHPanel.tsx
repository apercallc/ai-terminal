      ...
      <div className="ssh-panel" onClick={(e) => e.stopPropagation()}>
        <div className="ssh-header">
          ...
          <div className="ssh-header-actions">
            ...
            <button className="text-btn" onClick={handleImport}>Import</button>
            <button className="text-btn" onClick={handleExport}>Export</button>
          ...
          <div className="ssh-add-form">
            <div className="ssh-form-grid">
              <div className="settings-field">
                <label>Name</label>
                <input type="text" value={form.name || ""} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="e.g. Production Server" />
              </div>
              <div className="settings-field">
                <label>Host</label>
                <input type="text" value={form.host || ""} onChange={(e) => setForm((p) => ({ ...p, host: e.target.value }))} placeholder="e.g. 192.168.1.100" />
              </div>
              <div className="settings-field">
                <label>Port</label>
                <input type="number" value={form.port || 22} onChange={(e) => setForm((p) => ({ ...p, port: parseInt(e.target.value) || 22 }))} />
              </div>
              <div className="settings-field">
                <label>Username</label>
                <input type="text" value={form.username || ""} onChange={(e) => setForm((p) => ({ ...p, username: e.target.value }))} placeholder="e.g. root" />
              </div>
              <div className="settings-field full-width">
                <label>Private Key Path (optional)</label>
                <input type="text" value={form.privateKeyPath || ""} onChange={(e) => setForm((p) => ({ ...p, privateKeyPath: e.target.value }))} placeholder="e.g. ~/.ssh/id_rsa" />
              </div>
              <label className="export-checkbox">
                <input type="checkbox" checked={form.authMethod === "key"} onChange={(e) => setForm((p) => ({ ...p, authMethod: e.target.checked ? "key" : "password" }))} />
        ...
        <div className="ssh-body">
          {connections.length === 0 ? (
            <div className="ssh-empty">
              No SSH connections saved. Add one to get started.
            </div>
          ) : (
              ...
              {connections.map((conn) => (
                <div key={conn.id} className="ssh-item">
                  ...
                  <div className="ssh-actions">
                    <button className="ssh-connect-btn" onClick={() => handleConnect(conn)} title="Connect">
                      ▶ Connect
                    </button>
                    <button className="rec-btn" onClick={() => handleEdit(conn)} title="Edit">
                      ✎
                    </button>
                    <button className="rec-btn delete" onClick={() => handleDelete(conn.id)} title="Delete">
                      ×
                    </button>
                  </div>
                </div>
